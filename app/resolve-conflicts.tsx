...
import {
  getConflictWithEntityDetails,
  markSyncSuccess,
} from "@/services/models/SyncQueueModel";
import { markEntitySynced } from "@/services/models/GenericModel";
import { resolveEntityConflict } from "@/services/api/apiClient";
// import { reconcileUserAfterServerUpdate, User  } from "@/services/models/LocationsModel";
import { reconcileUserAfterServerUpdate, User } from "@/services/models/UserModel";
// Components
import {
  EntityInfo,
  DataComparisonTable,
...
export default function ResolveConflictsPage() {
  ...
  const { activeTab, handleTabPress } = useFooterNavigation("home", () =>
  ...
  const shouldShowUniqueConstraintForm =
  ...
  useEffect(() => {
    async function fetchData() {
      ...
      const syncId =
        typeof params.syncId === "string" ? params.syncId : undefined;
      ...
      if (rawData && rawData.conflict) {
        ...
        try {
          ...
          console.warn(
            "Failed to parse allowed strategies, using defaults:",
            e
          );
          setAllowedStrategies([
            "client_wins",
  ...
  useEffect(() => {
    if (selectedResolution && allowedStrategies.length > 0) {
      const strategy =
        selectedResolution === "local" ? "client_wins" : "server_wins";
  ...
  const handleConfirmResolution = async () => {
    ...
    if (!allowedStrategies.includes(strategy)) {
      Alert.alert(
        "Error",
        `The "${strategy}" strategy is not allowed for this conflict`
      );
      return;
    }
    ...
    try {
      ...
      if (response.success) {
        if (conflictData.conflict?.entity_type === "user") {
          await reconcileUserAfterServerUpdate(conflictData.serverData as User);
        }
        ...
        Alert.alert(
          "Success",
          `The conflict has been resolved using ${
            selectedResolution === "local" ? "your local data" : "server data"
          }.`,
  ...
  const handleUniqueConstraintResolution = async () => {
    ...
    try {
      ...
      if (response.success) {
        markSyncSuccess(conflictData.conflict?.sync_id ?? "");
        markEntitySynced(entityType, entityId); // Universal fix: mark entity as synced
        const changedFields = uniqueConstraintFields
          .map((field) => `${formatFieldName(field)}: ${editableValues[field]}`)
          .join(", ");
        Alert.alert(
          "Success",
        ...
        Alert.alert(
          "Error",
          response.message || "Failed to resolve unique constraint conflict"
        );
      }
    } catch (error) {
      ...
      Alert.alert(
        "Error",
        "Failed to resolve unique constraint conflict. Please try again."
      );
    } finally {
      setIsResolving(false);
    }
    ...
    return (
      <SafeAreaView style={{ flex: 1, backgroundColor: "#fff" }}>
        ...
        <View style={conflictStyles.errorContainer}>
          ...
          <Text style={conflictStyles.errorTitle}>
            No Conflict Data Available
          </Text>
          <Text style={conflictStyles.errorMessage}>
            The conflict data could not be loaded. Please try again or contact
            support.
          </Text>
          <TouchableOpacity
            style={conflictStyles.backButton}
            onPress={() => router.back()}
          >
        ...
        <TouchableWithoutFeedback onPress={Keyboard.dismiss}>
          ...
          >
                ...
                !isClientWinsAllowed && (
                  <View style={conflictStyles.warningContainer}>
                    ...
                    <Text style={conflictStyles.warningText}>
                      This is a unique constraint conflict, but neither "Update
                      Data" nor "Use My Data" strategies are allowed. Please
                      choose from the available standard resolution options.
                    </Text>
                  </View>
                )}
            </View>
            ...
            <ActionButtons
              ...
              onConfirm={
                shouldShowUniqueConstraintForm
                  ? handleUniqueConstraintResolution
              ...
              isDisabled={
                shouldShowUniqueConstraintForm
                  ? uniqueConstraintFields.some(
                      (field) => !editableValues[field]?.trim()
                    ) || isResolving
                  : !selectedResolution ||
                    isResolving ||
                    (selectedResolution === "local" &&
                      !allowedStrategies.includes("client_wins")) ||
                    (selectedResolution === "server" &&
                      !allowedStrategies.includes("server_wins"))
              }
            />
          </ScrollView>
        </TouchableWithoutFeedback>
